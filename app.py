    ...
    html.Div([
        dcc.Tabs([
            ...
            dcc.Tab(label='Bills', children=[
                dcc.Markdown('The following words and phrases are often used in this person\'s sponsored bills:'),
                dcc.Graph(id = 'tfidf')
            ]),
            dcc.Tab(label='News', children=[
                dcc.Markdown('The following summary is generated by ChatGPT from the headlines of current articles in which this person is mentioned:'),
                html.Div(id = 'news')
            ]),
...
@app.callback([Output(component_id = 'tfidf', component_property = 'figure')],
             [Input(component_id = 'dropdown', component_property = 'value')])

def tfidf(b):
    df = ct.charwords(b, host='postgres')
    return [ff.create_table(df)]

@app.callback([Output(component_id = 'news', component_property = 'children')],
             [Input(component_id = 'dropdown', component_property = 'value')])

def news(b):
    return [ct.summarize_news(b, host='postgres')]
